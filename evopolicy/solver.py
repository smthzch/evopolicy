...
class EvoSolver:
    ...
    def pathfind(self, particle=None, limit=None):
        ...
        while not done:
            if i==limit:
                break
            action = self.select_action(state, particle)
    ...
    def select_action(self, state, particle=None):
...
class MultiEvoSolver:
    def __init__(
        ...
        self.state_space = 1
        self.obs_disc = False
        if isinstance(self.wrap.env.observation_space, gym.spaces.discrete.Discrete):
            self.state_space = self.wrap.env.observation_space.n
            self.obs_disc = True
        else:
            #flatten observation space
            for i in range(len(self.wrap.env.observation_space.shape)):
                self.state_space *= self.wrap.env.observation_space.shape[i]
    ...
    def pathfind(self, particle=None, limit=None):
        ...
        while not done:
            if i==limit:
                break
            for agent_state in self.wrap.agents():
                action = self.select_action(agent_state, particle)
